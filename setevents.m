function EEG = setevents(EEG,FilteredForAccuracy,true_target,cat_filt,filename2)
...
while (thisevent < length(types)-50)
    ...
    if(types(thisevent) == 13)
        ...
        falsecat_present = 0;
        ignorefiltercheck = 1;
        if (length(cat_filt) > 0)
            truecat_present =  sum(cat_filt==true_cat_trigger)>0;
            falsecat_present =  sum(cat_filt==false_cat_trigger)>0;
            ignorefiltercheck = 0;
        end
        
        for evadd = firstpossibleevent: firstpossibleevent + 35
            if true_target
                if truecat_present || ignorefiltercheck
                    %True Targets
                ...
                if falsecat_present || ignorefiltercheck
                    %False Targets
